...
TEST_NUMBERS_VIEW_CACHE_TTL_SECONDS = max(20.0, float(str(_get("TEST_NUMBERS_VIEW_CACHE_TTL_SECONDS", "180") or "180").strip() or "180"))
TEST_NUMBERS_COUNTRIES_PER_PAGE = max(8, min(30, int(str(_get("TEST_NUMBERS_COUNTRIES_PER_PAGE", "15") or "15"))))
test_numbers_state: Dict[int, Dict] = {}
_test_numbers_cache: Dict[str, Any] = {"data": {}, "timestamp": 0.0}


def _classify_test_numbers_plan(row: Dict[str, Any]) -> str:
    blob = " | ".join([
        str(row.get("package") or ""),
        str(row.get("plan") or ""),
        str(row.get("bundle") or ""),
    ...
    normalized_blob = re.sub(r"\s+", " ", blob.lower())
    monthly_keywords = (
        "monthly", "month", "30 day", "30-day", "30days", "month plan",
        "شهري", "شهريه", "شهرية", "شهر", "30 يوم", "30يوم",
    )
    weekly_keywords = (
        "weekly", "week", "7 day", "7-day", "7days", "week plan",
        "أسبوع", "اسبوع", "أسبوعي", "اسبوعي", "أسبوعية", "اسبوعية", "7 أيام", "7ايام",
    )
    if any(keyword in normalized_blob for keyword in monthly_keywords):
        return "monthly"
    if any(keyword in normalized_blob for keyword in weekly_keywords):
        return "weekly"
    return "other"


def _extract_numbers_from_test_numbers_row(raw_row: Any, cells: List[Any], combined_text: str) -> List[str]:
    found: List[str] = []
    seen = set()
    ...
    for candidate in direct_candidates:
        ...
        if not text_value:
            continue
        _push(text_value)
        for match in _extract_phone_candidates_from_text(text_value):
            _push(match)
    return found


def _fetch_numbers_from_test_numbers_page(session: Optional[requests.Session] = None) -> List[Dict]:
    session = session or _build_site_session()
    page_url = _get("TEST_NUMBERS_URL", f"{SITE_URL}/test/numbers")
    data_url = _get("TEST_NUMBERS_DATA_URL", f"{SITE_URL}/test/numbers/data")
    ...
    for raw_row in payload.get("data", []) or []:
        ...
        package_text = _strip_html_text(
            _row_lookup_first(raw_row, "package", "plan", "bundle", "offer", "category", "period", "duration", "type", "validity")
            or (cells[3] if len(cells) > 3 else "")
        )
        title_text = _strip_html_text(
            _row_lookup_first(raw_row, "title", "name", "label", "service")
            or (cells[1] if len(cells) > 1 else "")
        )
        guessed_platform = (
            _guess_platform_from_payload(
                _row_lookup_first(raw_row, "platform", "service", "range", "name", "title", "label"),
                range_name,
                package_text,
                combined_text,
            )
        ...
        for number in _extract_numbers_from_test_numbers_row(raw_row, cells, combined_text):
            base_item = {
                "number": number,
                "platform": guessed_platform,
                "site_section": "test/numbers",
                "source": "test_numbers_page",
                "added_at": now_label,
            ...
            item = _enrich_number_item(base_item)
            if not item:
                continue
            item["platform"] = _route_platform_for_site_row(item, fallback=guessed_platform)
            item["site_section"] = "test/numbers"
            item["source"] = "test_numbers_page"
            item["added_at"] = now_label
            item["package"] = package_text
            item["plan_kind"] = _classify_test_numbers_plan(item)
            dedupe_key = (
                item.get("number", ""),
                str(item.get("platform", "")).lower(),
                str(item.get("country_key", "")).strip(),
                item.get("plan_kind", "other"),
            )
...
def _build_test_numbers_dataset(refresh: bool = False) -> Dict[str, Any]:
    now_ts = time.time()
    cached = _test_numbers_cache.get("data") or {}
    cache_ts = float(_test_numbers_cache.get("timestamp") or 0.0)
    if not refresh and cached and (now_ts - cache_ts) < TEST_NUMBERS_VIEW_CACHE_TTL_SECONDS:
        return cached

    try:
        rows = _fetch_numbers_from_test_numbers_page(_build_site_session())
    except Exception as fetch_err:
        if cached:
            fallback_dataset = dict(cached)
            fallback_dataset["used_cached_fallback"] = True
            _test_numbers_cache["data"] = fallback_dataset
            _test_numbers_cache["timestamp"] = time.time()
            return fallback_dataset
        raise RuntimeError(f"تعذر فتح صفحة test/numbers: {fetch_err}")

    platform_counts: Dict[str, int] = {}
    for row in rows:
        platform_name = _normalize_platform(row.get("platform", "")) or GENERAL_PLATFORM_NAME
    ...
    dataset = {
        ...
        "platforms": [
            platform_name
            for platform_name, _count in sorted(platform_counts.items(), key=lambda kv: (-kv[1], kv[0]))
        ],
        "platform_counts": platform_counts,
        "created_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "page_url": _get("TEST_NUMBERS_URL", f"{SITE_URL}/test/numbers"),
        "source_label": "test/numbers",
        "total_numbers": len(rows),
        "used_cached_fallback": False,
    }
    _test_numbers_cache["data"] = dataset
    _test_numbers_cache["timestamp"] = time.time()
    return dataset


def _test_numbers_state_from_dataset(user_id: int, data: Dict[str, Any], previous_state: Optional[Dict] = None) -> Dict[str, Any]:
    previous = dict(previous_state or test_numbers_state.get(user_id, {}) or {})
    platforms = list(data.get("platforms", []) or [])
    selected_platform = str(previous.get("selected_platform") or "").strip()
    ...
    state = {
        ...
        "platforms": platforms,
        "platform_counts": dict(data.get("platform_counts", {}) or {}),
        "selected_platform": selected_platform,
        "selected_country_key": str(previous.get("selected_country_key") or "").strip() if selected_platform else "",
        "countries_page": int(previous.get("countries_page", 0) or 0),
        "created_at": str(data.get("created_at") or ""),
        "page_url": str(data.get("page_url") or _get("TEST_NUMBERS_URL", f"{SITE_URL}/test/numbers")),
        "source_label": str(data.get("source_label") or "test/numbers"),
        "total_numbers": int(data.get("total_numbers", 0) or 0),
        "used_cached_fallback": bool(data.get("used_cached_fallback")),
    }
    test_numbers_state[user_id] = state
    return state


def _test_numbers_selected_platform_rows(state: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    payload = state if isinstance(state, dict) else {}
    selected_platform = _normalize_platform(payload.get("selected_platform", ""))
    ...
    for raw_row in payload.get("rows", []) or []:
        if not isinstance(raw_row, dict):
            continue
        row = _enrich_number_item(raw_row)
        if not row:
            continue
        if _normalize_platform(row.get("platform", "")) != selected_platform:
            continue
        rows.append(row)
    return _dedupe_numbers(rows)


def _test_numbers_country_buckets(state: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    buckets: Dict[str, Dict[str, Any]] = {}
    for row in _test_numbers_selected_platform_rows(state):
        info = _country_info_from_row(row.get("number", ""), row)
        country_key = str(row.get("country_key") or info.get("key") or "unknown").strip() or "unknown"
        bucket = buckets.setdefault(country_key, {
            "key": country_key,
            "name": str(row.get("country_name_ar") or row.get("country_name") or info.get("name") or "غير محددة").strip() or "غير محددة",
            "display_name": str(row.get("country_name_ar") or row.get("country_name") or info.get("name") or "غير محددة").strip() or "غير محددة",
            "flag": str(row.get("country_flag") or info.get("flag") or "🌐").strip() or "🌐",
            "code": str(row.get("country_code") or info.get("code") or "").strip(),
            "rows": [],
            "weekly": 0,
            "monthly": 0,
            "other": 0,
        ...
        row_copy = dict(row)
        row_copy["plan_kind"] = _classify_test_numbers_plan(row_copy)
        bucket["rows"].append(row_copy)
        bucket[row_copy["plan_kind"] if row_copy["plan_kind"] in {"weekly", "monthly"} else "other"] += 1
        bucket["total"] += 1

    return sorted(
        buckets.values(),
        key=lambda bucket: (-int(bucket.get("total", 0) or 0), str(bucket.get("display_name") or ""), str(bucket.get("key") or "")),
    )


def _test_numbers_selected_country_bucket(state: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    payload = state if isinstance(state, dict) else {}
    selected_country_key = str(payload.get("selected_country_key") or "").strip()
    if not selected_country_key:
        return None
    for bucket in _test_numbers_country_buckets(payload):
        if str(bucket.get("key") or "").strip() == selected_country_key:
            return bucket
    return None


def _test_numbers_current_page(user_id: int) -> int:
    state = test_numbers_state.get(user_id, {})
    buckets = _test_numbers_country_buckets(state)
    total_pages = max(1, (len(buckets) + TEST_NUMBERS_COUNTRIES_PER_PAGE - 1) // TEST_NUMBERS_COUNTRIES_PER_PAGE)
...
def _test_numbers_plan_count(bucket: Optional[Dict[str, Any]], plan_kind: str) -> int:
    payload = bucket if isinstance(bucket, dict) else {}
    exact_count = int(payload.get(plan_kind, 0) or 0)
    if exact_count:
        return exact_count
    if int(payload.get("weekly", 0) or 0) == 0 and int(payload.get("monthly", 0) or 0) == 0:
        return int(payload.get("total", 0) or 0)
    return 0


def _build_test_numbers_platform_markup(user_id: int) -> types.InlineKeyboardMarkup:
    state = test_numbers_state.get(user_id, {})
    platforms = list(state.get("platforms", []) or [])
    counts = dict(state.get("platform_counts", {}) or {})
    mk = types.InlineKeyboardMarkup(row_width=2)
    for idx, platform_name in enumerate(platforms):
        icon = PLATFORM_BUTTON_ICONS.get(platform_name, "📂")
        count = counts.get(platform_name, 0)
        mk.add(types.InlineKeyboardButton(f"{icon} {platform_name} ({count})"[:60], callback_data=f"testnumplt_{idx}"))
    mk.add(types.InlineKeyboardButton("🔄 تحديث الصفحة", callback_data="testnum_refresh"))
    return mk


def _test_numbers_platforms_text(user_id: int) -> str:
    state = test_numbers_state.get(user_id, {})
    lines = [
        "🧪 صفحة test/numbers",
        "",
        f"🌐 الرابط: {state.get('page_url', _get('TEST_NUMBERS_URL', f'{SITE_URL}/test/numbers'))}",
        f"📚 المنصات المكتشفة: {len(state.get('platforms', []) or [])}",
        f"📱 إجمالي الأرقام المكتشفة: {state.get('total_numbers', 0)}",
        f"🕐 آخر تحديث: {state.get('created_at', '')}",
        "♻️ تم استخدام آخر نتائج محفوظة مؤقتاً" if state.get("used_cached_fallback") else "✅ البيانات مباشرة من صفحة test/numbers",
        "",
        "اختَر المنصة أولاً، وبعدها هيظهر لك كل الدول الموجودة داخل نفس المنصة.",
    ]
    return "\n".join(lines)


def _render_test_numbers_platforms(chat_id: int, user_id: int, message_id: Optional[int] = None) -> None:
    state = test_numbers_state.get(user_id, {})
    if not list(state.get("platforms", []) or []):
        _send_or_edit(
            chat_id,
            "⚠️ لم أجد منصات داخل صفحة test/numbers حالياً. جرّب تسجيل الدخول للموقع أو تحديث الكوكيز ثم أعد المحاولة.",
            message_id=message_id,
        )
        return
    _send_or_edit(
        chat_id,
        _test_numbers_platforms_text(user_id),
        reply_markup=_build_test_numbers_platform_markup(user_id),
        message_id=message_id,
    )


def _build_test_numbers_country_markup(user_id: int) -> types.InlineKeyboardMarkup:
    state = test_numbers_state.get(user_id, {})
    buckets = _test_numbers_country_buckets(state)
    page = _test_numbers_current_page(user_id)
    total_pages = max(1, (len(buckets) + TEST_NUMBERS_COUNTRIES_PER_PAGE - 1) // TEST_NUMBERS_COUNTRIES_PER_PAGE)
    start = page * TEST_NUMBERS_COUNTRIES_PER_PAGE
    end = start + TEST_NUMBERS_COUNTRIES_PER_PAGE
    mk = types.InlineKeyboardMarkup(row_width=1)
    for idx, bucket in enumerate(buckets[start:end], start):
        label = f"{bucket.get('flag', '🌐')} {bucket.get('display_name', 'غير محددة')} ({bucket.get('total', 0)})"
        mk.add(types.InlineKeyboardButton(label[:60], callback_data=f"testnumcty_{idx}"))

    nav_buttons = []
    if page > 0:
        nav_buttons.append(types.InlineKeyboardButton("⬅️ السابق", callback_data=f"testnumctypg_{page - 1}"))
    if total_pages > 1:
        nav_buttons.append(types.InlineKeyboardButton(f"📄 {page + 1}/{total_pages}", callback_data=f"testnumctypg_{page}"))
    if page < total_pages - 1:
        nav_buttons.append(types.InlineKeyboardButton("التالي ➡️", callback_data=f"testnumctypg_{page + 1}"))
    if nav_buttons:
        mk.row(*nav_buttons)

    mk.add(types.InlineKeyboardButton("⬅️ رجوع للمنصات", callback_data="testnum_back_platforms"))
    mk.add(types.InlineKeyboardButton("🔄 تحديث الصفحة", callback_data="testnum_refresh"))
    return mk


def _render_test_numbers_countries(chat_id: int, user_id: int, message_id: Optional[int] = None) -> None:
    state = test_numbers_state.get(user_id, {})
    selected_platform = str(state.get("selected_platform") or "").strip()
    if not selected_platform:
        _render_test_numbers_platforms(chat_id, user_id, message_id=message_id)
        return
    buckets = _test_numbers_country_buckets(state)
    if not buckets:
        _send_or_edit(
            chat_id,
            f"⚠️ لم أجد دولاً داخل منصة {selected_platform} في صفحة test/numbers حالياً.",
            reply_markup=_build_test_numbers_platform_markup(user_id),
            message_id=message_id,
        )
        return
    ...
    lines = [
        "🧪 صفحة test/numbers",
        "",
        f"📂 المنصة المختارة: {_display_platform_name(selected_platform)}",
        f"🌍 الدول المتاحة داخل المنصة: {len(buckets)}",
        f"📄 صفحة الدول الحالية: {page + 1}/{total_pages}",
        f"📱 إجمالي أرقام المنصة: {len(_test_numbers_selected_platform_rows(state))}",
        "",
        "اختَر الدولة من الأزرار تحت، وبعدها البوت هيسألك هل تريد أرقام أسبوعية أو شهرية.",
    ]
    _send_or_edit(
        chat_id,
        "\n".join(lines),
        reply_markup=_build_test_numbers_country_markup(user_id),
        message_id=message_id,
    )


def _build_test_numbers_plan_markup(user_id: int) -> types.InlineKeyboardMarkup:
    state = test_numbers_state.get(user_id, {})
    bucket = _test_numbers_selected_country_bucket(state) or {}
    weekly_count = _test_numbers_plan_count(bucket, "weekly")
    monthly_count = _test_numbers_plan_count(bucket, "monthly")
    mk = types.InlineKeyboardMarkup(row_width=2)
    mk.add(
        types.InlineKeyboardButton(f"📅 أسبوعية ({weekly_count})"[:60], callback_data="testnumplan_weekly"),
        types.InlineKeyboardButton(f"🗓️ شهرية ({monthly_count})"[:60], callback_data="testnumplan_monthly"),
    )
    mk.add(types.InlineKeyboardButton("⬅️ رجوع للدول", callback_data="testnum_back_countries"))
    mk.add(types.InlineKeyboardButton("⬅️ رجوع للمنصات", callback_data="testnum_back_platforms"))
    mk.add(types.InlineKeyboardButton("🔄 تحديث الصفحة", callback_data="testnum_refresh"))
    return mk


def _render_test_numbers_plans(chat_id: int, user_id: int, message_id: Optional[int] = None) -> None:
    state = test_numbers_state.get(user_id, {})
    selected_platform = str(state.get("selected_platform") or "").strip()
    bucket = _test_numbers_selected_country_bucket(state)
    if not selected_platform or not bucket:
        _render_test_numbers_countries(chat_id, user_id, message_id=message_id)
        return
    ...
    lines = [
        ...
        f"🌍 الدولة: {bucket.get('flag', '🌐')} {bucket.get('display_name', 'غير محددة')}",
        f"☎️ مفتاح الدولة: {bucket.get('code', 'غير محدد') or 'غير محدد'}",
        f"📱 إجمالي الأرقام المتاحة: {bucket.get('total', 0)}",
        f"📅 المتاح كأسبوعي: {weekly_count}",
        f"🗓️ المتاح كشهري: {monthly_count}",
        "",
        "اختَر نوع الإضافة، وسيتم إرسال ملف TXT فيه كل الأرقام المختارة سطراً بسطر حتى تقدر تنسخها وتضيفها يدوياً بدون مشاكل.",
    ]
    if int(bucket.get("weekly", 0) or 0) == 0 and int(bucket.get("monthly", 0) or 0) == 0:
        lines.extend(["", "ℹ️ الصفحة لا تميز بين أسبوعي/شهري بشكل واضح، لذلك سيستخدم البوت كل أرقام الدولة مع الخيار الذي تضغطه."])
    _send_or_edit(
        chat_id,
        "\n".join(lines),
        reply_markup=_build_test_numbers_plan_markup(user_id),
        message_id=message_id,
    )


def _filter_test_numbers_rows_by_plan(rows: List[Dict[str, Any]], plan_kind: str) -> List[Dict[str, Any]]:
    exact_rows: List[Dict[str, Any]] = []
    has_exact_classification = False
    for row in rows or []:
        row_copy = dict(row)
        row_plan_kind = _classify_test_numbers_plan(row_copy)
        row_copy["plan_kind"] = row_plan_kind
        if row_plan_kind in {"weekly", "monthly"}:
            has_exact_classification = True
        if row_plan_kind == plan_kind:
            exact_rows.append(row_copy)
    if exact_rows:
        return _dedupe_numbers(exact_rows)
    if not has_exact_classification:
        return _dedupe_numbers([dict(row) for row in rows or [] if isinstance(row, dict)])
    return []


def _send_test_numbers_txt_file(chat_id: int, platform: str, bucket: Dict[str, Any], plan_kind: str, rows: List[Dict[str, Any]]) -> None:
    import io

    plan_label = "weekly" if plan_kind == "weekly" else "monthly"
    ...
    safe_platform = re.sub(r"[^A-Za-z0-9_-]+", "_", _normalize_platform(platform) or "platform").strip("_") or "platform"
    safe_country = re.sub(r"[^A-Za-z0-9_-]+", "_", str(bucket.get("display_name") or bucket.get("name") or "country")).strip("_") or "country"
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    file_name = f"test_numbers_{safe_platform}_{safe_country}_{plan_label}_{timestamp}.txt"
    file_body = "\n".join(unique_numbers).encode("utf-8")
    caption = (
        f"📄 ملف TXT جاهز للنسخ اليدوي\n"
        f"📂 المنصة: {_display_platform_name(platform)}\n"
        f"🌍 الدولة: {bucket.get('flag', '🌐')} {bucket.get('display_name', 'غير محددة')}\n"
        f"🗂️ النوع: {'أسبوعية' if plan_kind == 'weekly' else 'شهرية'}\n"
        f"📱 عدد الأرقام داخل الملف: {len(unique_numbers)}"
    )
    bot.send_document(
        chat_id,
        (file_name, io.BytesIO(file_body), "text/plain"),
        caption=caption,
    )


def dev_test_numbers_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    bot.answer_callback_query(call.id, "⏳ جاري فتح صفحة test/numbers...")
    _send_or_edit(
        call.message.chat.id,
        "⏳ جاري فتح صفحة test/numbers وجلب المنصات والدول المتاحة...",
        message_id=call.message.message_id,
    )
    try:
        previous_state = dict(test_numbers_state.get(call.from_user.id, {}) or {})
        dataset = _build_test_numbers_dataset(refresh=True)
        _test_numbers_state_from_dataset(call.from_user.id, dataset, previous_state=previous_state)
        _render_test_numbers_platforms(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)
    except Exception as test_numbers_err:
        _send_or_edit(
            call.message.chat.id,
            f"❌ تعذر فتح صفحة test/numbers: {test_numbers_err}",
            message_id=call.message.message_id,
        )


def testnum_refresh_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    ...
    try:
        ...
        dataset = _build_test_numbers_dataset(refresh=True)
        _test_numbers_state_from_dataset(call.from_user.id, dataset, previous_state=previous_state)
        state = test_numbers_state.get(call.from_user.id, {})
        if state.get("selected_country_key"):
            _render_test_numbers_plans(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)
        elif state.get("selected_platform"):
            _render_test_numbers_countries(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)
        else:
            _render_test_numbers_platforms(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)
    except Exception as refresh_err:
        _send_or_edit(
            call.message.chat.id,
            f"❌ تعذر تحديث صفحة test/numbers: {refresh_err}",
            message_id=call.message.message_id,
        )


def testnum_platform_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    state = test_numbers_state.get(call.from_user.id, {})
    platforms = list(state.get("platforms", []) or [])
    try:
        idx = int(call.data.replace("testnumplt_", "", 1))
    ...
    state["countries_page"] = 0
    test_numbers_state[call.from_user.id] = state
    bot.answer_callback_query(call.id, f"✅ تم اختيار {selected_platform}")
    _render_test_numbers_countries(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)


def testnum_country_page_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    state = test_numbers_state.get(call.from_user.id, {})
    try:
        page = int(call.data.replace("testnumctypg_", "", 1))
    except Exception:
        page = 0
    state["countries_page"] = max(0, page)
    test_numbers_state[call.from_user.id] = state
    bot.answer_callback_query(call.id, "✅ تم تغيير الصفحة")
    _render_test_numbers_countries(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)


def testnum_country_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    state = test_numbers_state.get(call.from_user.id, {})
    buckets = _test_numbers_country_buckets(state)
    try:
        idx = int(call.data.replace("testnumcty_", "", 1))
        bucket = buckets[idx]
    except Exception:
        bot.answer_callback_query(call.id, "الدولة غير موجودة", show_alert=True)
        return

    state["selected_country_key"] = str(bucket.get("key") or "").strip()
    test_numbers_state[call.from_user.id] = state
    bot.answer_callback_query(call.id, f"✅ تم اختيار {bucket.get('display_name', 'الدولة')}")
    _render_test_numbers_plans(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)


def testnum_back_platforms_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    state = test_numbers_state.get(call.from_user.id, {})
    ...
    state["countries_page"] = 0
    test_numbers_state[call.from_user.id] = state
    bot.answer_callback_query(call.id, "تم الرجوع للمنصات")
    _render_test_numbers_platforms(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)


def testnum_back_countries_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return
    state = test_numbers_state.get(call.from_user.id, {})
    state["selected_country_key"] = ""
    test_numbers_state[call.from_user.id] = state
    bot.answer_callback_query(call.id, "تم الرجوع للدول")
    _render_test_numbers_countries(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)


def testnum_plan_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "غير مصرح", show_alert=True)
        return

    plan_kind = call.data.replace("testnumplan_", "", 1).strip().lower()
    if plan_kind not in {"weekly", "monthly"}:
        bot.answer_callback_query(call.id, "النوع غير معروف", show_alert=True)
        return

    state = test_numbers_state.get(call.from_user.id, {})
    selected_platform = _normalize_platform(state.get("selected_platform", ""))
    bucket = _test_numbers_selected_country_bucket(state)
    if not selected_platform or not bucket:
        bot.answer_callback_query(call.id, "اختر المنصة ثم الدولة أولاً", show_alert=True)
        _render_test_numbers_platforms(call.message.chat.id, call.from_user.id, message_id=call.message.message_id)
        return

    target_rows = _filter_test_numbers_rows_by_plan(list(bucket.get("rows", []) or []), plan_kind)
    if not target_rows:
        bot.answer_callback_query(call.id, "لا توجد أرقام مطابقة لهذا النوع حالياً", show_alert=True)
        return
    ...
    for row in target_rows:
        ...
        item["site_section"] = "test/numbers"
        item["source"] = f"test_numbers_{plan_kind}"
        item["added_at"] = now_label
        item["plan_kind"] = plan_kind
        prepared.append(item)

    prepared = _dedupe_numbers(prepared)
    if not prepared:
        bot.answer_callback_query(call.id, "لا توجد أرقام صالحة للإضافة", show_alert=True)
        return

    before = list(numbers_db.get("numbers", []))
    merged = _append_numbers(prepared)
    added_items = [
        item for item in _find_newly_added_numbers(before, merged)
        if _normalize_platform(item.get("platform", "")) == selected_platform
        and _normalize_number(item.get("number", "")) in selected_numbers
    ]
    added_count = len(_dedupe_numbers(added_items))
    existing_count = max(0, len(prepared) - added_count)
    plan_label_ar = "أسبوعية" if plan_kind == "weekly" else "شهرية"

    if added_items:
        _notify_grouped_country_adds(added_items, source=f"test_numbers_{plan_kind}")
        _notify_admin_site_country_add(
            str(bucket.get("display_name") or bucket.get("name") or "غير محددة"),
            str(bucket.get("flag") or "🌐"),
            selected_platform,
            added_count,
            0,
            len(prepared),
    ...
    _send_test_numbers_txt_file(call.message.chat.id, selected_platform, bucket, plan_kind, prepared)
    bot.answer_callback_query(call.id, "✅ تم تجهيز الملف")
    summary_lines = [
        "✅ تم تجهيز أرقام صفحة test/numbers بنجاح",
        "",
        f"📂 المنصة: {_display_platform_name(selected_platform)}",
        f"🌍 الدولة: {bucket.get('flag', '🌐')} {bucket.get('display_name', 'غير محددة')}",
        f"🗂️ النوع المختار: {plan_label_ar}",
        f"📱 الأرقام داخل الملف: {len(prepared)}",
        f"🆕 الجديد المضاف داخل البوت: {added_count}",
        f"♻️ الموجود مسبقاً داخل البوت: {existing_count}",
        "",
        "تم إرسال ملف TXT منفصل يحتوي الأرقام فقط سطراً بسطر حتى تقدر تنسخها وتضيفها يدوياً بدون مشاكل.",
    ]
    _send_or_edit(
        call.message.chat.id,
        "\n".join(summary_lines),
        reply_markup=_build_test_numbers_plan_markup(call.from_user.id),
        message_id=call.message.message_id,
    )
...
def _build_developer_panel_markup() -> types.InlineKeyboardMarkup:
    ...
    mk.add(
        types.InlineKeyboardButton('🧪 فتح test/numbers', callback_data='dev_test_numbers'),
    )
    return mk
...
def _developer_commands_text() -> str:
    return (
        ...
        'وكمان تقدر تفتح test/numbers ثم تختار المنصة ← الدولة ← أسبوعية/شهرية، وسيصلك ملف TXT فيه كل الأرقام المختارة للنسخ اليدوي بدون مشاكل. '
        'وتقدر كذلك تفتح my/ranges و my/messages مباشرة من اللوحة بعد تسجيل الدخول.'
    )
...
def _dispatch_callback_query(call):
    ...
    exact_map = {
        ...
        "dev_test_numbers": dev_test_numbers_callback,
        ...
        "testnum_refresh": testnum_refresh_callback,
        "testnum_back_platforms": testnum_back_platforms_callback,
        "testnum_back_countries": testnum_back_countries_callback,
    }
    ...
    prefix_map = [
        ...
        ("testnumplt_", testnum_platform_callback),
        ("testnumctypg_", testnum_country_page_callback),
        ("testnumcty_", testnum_country_callback),
        ("testnumplan_", testnum_plan_callback),
    ]
